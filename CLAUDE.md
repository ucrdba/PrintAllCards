...
Seven top-level modules wired together by `main.py`:
...
- **`dom_driver.py`** (`DomDriver`) — drives Schoolhouse Smiles through the Chrome DevTools Protocol over `websocket-client` (no Playwright). Connects to `127.0.0.1:<dom_debug_port>`, refuses the port unless `/json/version`'s User-Agent contains `schoolhouse-smiles`, and finds controls by visible label/text only (`mat-form-field` label, exact button text, `mat-radio-button` text) — never by the generated Angular Material ids (`mat-input-2`, `mat-radio-10-input`), which change between launches. Clicks and typing are CDP `Input.*` events at page (CSS-pixel) coordinates computed from the element, not screen coordinates. One socket is shared by the GUI and automation threads, guarded by an `RLock`. Card types are read only from the `mat-radio-group` headed `ID Card` (other radio groups, like the Local/Network printer choice, share the page). The printer Schoolhouse Smiles prints to is not in the DOM or its `settings.json`: `get_print_server_printers()` reads the enabled printers from its local print server (`PrintService.exe`, `GET http://127.0.0.1:45696/api/settings`), and `_refresh_printer_list` prefers that printer over the saved `selected_printer`.
...
**DOM control (default, `config.use_dom_control`).** When `dom_ready()` connects, `_execute_single_student_dom` runs instead of everything below: type the ID into the `Student Search` field (`Input.insertText`) and confirm its value → wait up to 5s for the `mat-option` whose text holds the ID as a whole token (so `1234567` never matches `12345678`; none means the student is not in Schoolhouse Smiles: fail with `last_failure_kind = FAILURE_NOT_FOUND`, which the batch loop auto-skips and lists in the end-of-run summary) → **click that exact option** (not Enter, which picks the first option and can load a different student sharing the prefix) → wait up to `max_search_wait` until the **`Student ID` field** equals the ID (the real proof the right record loaded) → Card Type by name (`config.card_type_name`, verified selected; blank name falls back to the Card Type coordinates) → the button whose text is exactly `Print` (fails if missing/disabled; never falls back to coordinates) → read `(Last Printed ...)` before/after to log confirmation. Schoolhouse Smiles must run with `--remote-debugging-port` (the GUI's "Launch in DOM Mode" starts it with that and `--force-renderer-accessibility`). If DOM control is enabled but unavailable, `dom_ready()` logs the reason once and the coordinate flow below is used.

**Coordinate fallback.** Queue-sync gate (block while the printer's `cJobs` >= `max_queue_jobs`) → click search box → `Ctrl+A`/Backspace → clipboard copy + `Ctrl+V` → confirm the search box holds exactly the ID (`_enter_student_id`/`read_search_box_text`: UIAutomation value of the focused edit, else a Ctrl+A/Ctrl+C read-back guarded by a clipboard sentinel; two paste attempts then one `pyautogui.write`, and the student fails before Enter if it never matches) → Enter → optional verification → Print. The Print trigger has a three-way fallback: a UIAutomation-located element, else the configured fixed coordinates, else `config.print_hotkey`. Card Type is handled by `click_card_type()` immediately before the Print step: it clicks the configured location for **every** student when the GUI's "Required" checkbox (`config.card_type_required`) is ticked, and is a complete no-op when it is not (there is no once-per-batch click).
...
- **In the coordinate fallback, the Print button is located by name, everything else by coordinates.** Without `--force-renderer-accessibility`, Chromium hands UIAutomation a stale tree (it can still show "No student is selected" while a student is open), so this lookup silently fails and falls back to the fixed coordinates — which is why DOM control exists. `find_print_button()` walks the target window's UIAutomation tree (`auto.WalkControl` — note that `GetDescendants` does not exist in `uiautomation` 2.0.29) and returns the centre of the `ButtonControl` named exactly `Print`. This is necessary because a student who has been printed before gains a "(Last Printed ...)" line that pushes the button down the Y axis by a couple of hundred pixels. An exact-name match always wins over a prefix match so `Print with Dialog` is never clicked, the candidate window is confirmed by process name (`schoolhouse-smiles.exe`) rather than title, and any failure falls back to `config.print_x/print_y`. Search box and Card Type deliberately stay on fixed coordinates. `uiautomation`/`pywin32` are Windows-only and imported defensively (`HAS_UIAUTOMATION`, and `win32print` is likewise guarded in `gui.py`).
